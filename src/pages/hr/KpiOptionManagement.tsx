...
interface DepartmentSectionProps {
  ...
  onResolvedChange: (next: CodeOption[]) => void
}
...
function DepartmentLevelSection({ title, level, onLevelChange, onResolvedChange }: DepartmentSectionProps) {
  ...
  // 트리 로딩 후 resolved가 바뀌면 부모 draft에 반영
  useEffect(() => {
    if (!loading && tree.length > 0) onResolvedChange(resolved)
...
export default function KpiOptionManagement() {
  ...
  // 조직도 로딩 후 자동 resolve 결과 반영 (dirty 플래그는 토글하지 않음 — 사용자 입력이 아님)
  const updateResolvedDepts = (next: CodeOption[]) => {
    setDraft(d => {
      const changed = d.departments.length !== next.length ||
        d.departments.some((x, i) => x.code !== next[i]?.code || x.label !== next[i]?.label)
      if (!changed) return d
  ...
  const handleSave = () => {
    ...
    const clean: KpiOptionsState = {
      ...
      departmentLevel: draft.departmentLevel,
    ...
    <div className="flex-1 overflow-y-auto p-6">
      ...
      <div className="space-y-5">
        ...
        <DepartmentLevelSection
          title="② 적용 부서"
          level={draft.departmentLevel}
          onLevelChange={v => update({ departmentLevel: v })}
          onResolvedChange={updateResolvedDepts}
        />
