...
import HolidayManageView from './attendance-policy/HolidayManageView'
type AttPolicyView = 'leave-rule' | 'leave-promotion' | 'legal-leave' | 'holiday' | 'overtime-settings' | 'work-group' | 'allowed-ip'

const ATT_POLICY_MENUS: { key: AttPolicyView; label: string; group: string }[] = [
  { key: 'leave-rule', label: '연차 발생 규칙 설정', group: '연차·휴가' },
  { key: 'leave-promotion', label: '연차 촉진', group: '연차·휴가' },
  { key: 'legal-leave', label: '별도 휴가 관리', group: '연차·휴가' },
  { key: 'holiday', label: '사내 휴일 관리', group: '연차·휴가' },
  ...
  return (
    <div className="flex gap-0 -m-6 h-[calc(100%+48px)]">
      ...
      <div className="flex-1 overflow-y-auto p-6">
        ...
        {view === 'holiday' && <HolidayManageView />}
