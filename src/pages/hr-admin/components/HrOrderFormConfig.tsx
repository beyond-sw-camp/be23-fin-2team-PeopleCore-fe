...
const DEFAULT_FIELDS: FieldConfig[] = [
  // ① 발령 기본 정보
  { fieldKey: 'orderDate',    label: '발령일자',     section: '발령 기본 정보', fieldType: 'DATE',   visible: true, required: true,  sortOrder: 1 },
  { fieldKey: 'orderNumber',  label: '발령번호',     section: '발령 기본 정보', fieldType: 'AUTO',   visible: true, required: true,  sortOrder: 2 },
  { fieldKey: 'orderTitle',   label: '발령제목',     section: '발령 기본 정보', fieldType: 'TEXT',   visible: true, required: true,  sortOrder: 3 },
  { fieldKey: 'orderCount',   label: '발령 인원',    section: '발령 기본 정보', fieldType: 'AUTO',   visible: true, required: false, sortOrder: 4 },

  // ② 발령 유형
  { fieldKey: 'orderType',    label: '발령유형',     section: '발령 유형',     fieldType: 'SELECT', visible: true, required: true,  sortOrder: 1, options: ['입사', '퇴사', '직위변경', '부서변경', '보직변경'] },

  // ③ 대상자 정보
  { fieldKey: 'empSearch',    label: '사원명',       section: '대상자 정보',   fieldType: 'SEARCH', visible: true, required: true,  sortOrder: 1 },
  { fieldKey: 'department',   label: '부서',         section: '대상자 정보',   fieldType: 'TEXT', visible: true, required: true,  sortOrder: 2, autoFillFrom: 'department' },
  { fieldKey: 'rank',         label: '직위',         section: '대상자 정보',   fieldType: 'TEXT', visible: true, required: true,  sortOrder: 3, autoFillFrom: 'rank' },
  { fieldKey: 'newDepartment',label: '변경 부서',    section: '대상자 정보',   fieldType: 'SELECT', visible: true, required: false, sortOrder: 4 },
  { fieldKey: 'newRank',      label: '변경 직위',    section: '대상자 정보',   fieldType: 'SELECT', visible: true, required: false, sortOrder: 5 },
  { fieldKey: 'newPosition',  label: '변경 보직',    section: '대상자 정보',   fieldType: 'SELECT', visible: true, required: false, sortOrder: 6 },
  { fieldKey: 'empOrderDate', label: '발령일',       section: '대상자 정보',   fieldType: 'DATE',   visible: true, required: true,  sortOrder: 7 },

  // ④ 기타
  { fieldKey: 'orderReason',  label: '발령 사유',    section: '기타',         fieldType: 'TEXTAREA', visible: true, required: false, sortOrder: 1 },
]

const SECTIONS = ['발령 기본 정보', '발령 유형', '대상자 정보', '기타']
...
export default function HrOrderFormConfig({ onBack }: Props) {
  ...
  useEffect(() => {
    formSetupApi.getSetup('HR_ORDER')
      .then(res => {
  ...
  const handleSave = () => {
    ...
    formSetupApi.saveSetup('HR_ORDER', reqs)
      .then(res => {
        const list: FieldConfig[] = []
        for (const item of res.data) { list.push(resToFieldConfig(item)) }
        if (list.length > 0) setFields(list)
  ...
  const handleReset = () => {
    ...
    formSetupApi.resetSetup('HR_ORDER')
      .then(res => {
        const list: FieldConfig[] = []
        for (const item of res.data) { list.push(resToFieldConfig(item)) }
        if (list.length > 0) setFields(list)
  ...
  return (
    <div>
      <div className="flex items-center gap-3 mb-1">
        ...
        <h3 className="text-[16px] font-bold text-gray-800">인사발령 등록 폼 설정</h3>
      </div>
      <p className="text-[12px] text-gray-400 mb-5 ml-10">인사발령 등록 시 입력받을 항목을 설정합니다. 회사 상황에 맞게 필드를 커스텀할 수 있습니다.</p>
      ...
      <div className="bg-blue-50 rounded-xl p-3.5 mb-5 flex items-start gap-2.5">
        ...
        <div className="text-[11px] text-blue-700 space-y-0.5">
          <p>필드를 <strong className="font-semibold">숨김</strong> 처리하면 인사발령 등록 화면에서 해당 항목이 표시되지 않습니다.</p>
        </div>
      </div>
          ...
          <div className="relative bg-white rounded-xl shadow-xl w-[440px]">
            ...
            <div className="px-6 py-5 space-y-4">
              <div className="flex items-center gap-3">
                ...
                <input type="text" value={addForm.label} onChange={e => setAddForm(prev => ({ ...prev, label: e.target.value }))} placeholder="예) 발령 근거" className="flex-1 text-[12px] border border-gray-200 rounded px-2.5 py-1.5 outline-none focus:border-[#1D9E75]" autoFocus />
              </div>
              <div className="flex items-center gap-3">
                <label className="text-[12px] text-gray-500 w-20 shrink-0">필드 키 <span className="text-red-500">*</span></label>
                <input type="text" value={addForm.fieldKey} onChange={e => setAddForm(prev => ({ ...prev, fieldKey: e.target.value.replace(/[^a-zA-Z0-9_]/g, '') }))} placeholder="예) orderBasis" className="flex-1 text-[12px] border border-gray-200 rounded px-2.5 py-1.5 outline-none focus:border-[#1D9E75] font-mono" />
              </div>
              <div className="flex items-center gap-3">
      ...
      {showPreview && (
        <FormPreviewModal title="인사발령 등록" sections={SECTIONS} fields={fields} onClose={() => setShowPreview(false)} />
      )}
    </div>
  )
}
